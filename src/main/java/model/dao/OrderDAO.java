...
public class OrderDAO {
	...
	private final static String DELETE_GHOST_ORDERS_BY_ACCOUNT_PK =
			"DELETE FROM ORDERS O "
			+ "WHERE NOT EXISTS ("
			+ "    SELECT 1 "
			+ "    FROM ORDERS_ITEM OI "
			+ "    WHERE OI.ORDERS_PK = O.ORDERS_PK"
			+ ")";
	...
	public boolean delete(OrderDTO orderDTO) {
		...
		try{
			...
			else if(orderDTO.getCondition().equals("DELETE_GHOST_ORDERS_BY_ACCOUNT_PK")) {
				pstmt = conn.prepareStatement(DELETE_GHOST_ORDERS_BY_ACCOUNT_PK);
				pstmt.setInt(1, orderDTO.getAccountPk());
				int result = pstmt.executeUpdate();
				if(result < 0) {
					System.out.println("[로그] delete 실패, orderDTO = "+orderDTO);
	...
	private boolean update(OrderDTO orderDTO) {
		return false;
	}
}
